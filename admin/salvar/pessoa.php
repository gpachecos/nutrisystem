<?php
	...
	if ( $_POST ) {
		...
		if ( empty ( $idpessoa ) ) {
			...
			$sql = "select idpessoa, nome from pessoa where cpf = ? limit 1";
			...
			$sql = "select idpessoa, nome from pessoa where cpf = ? and idpessoa <> ? limit 1";
			$consulta = $pdo->prepare( $sql );
			$consulta->bindParam(1,$cpf);
			$consulta->bindParam(2,$idpessoa);
